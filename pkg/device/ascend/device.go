...
func InitDevices() []*Devices {
	...
	for _, vnpu := range config.VNPUs {
		...
		util.HandshakeAnnos[commonWord] = fmt.Sprintf("hami.io/node-handshake-%s", commonWord)
...
func ParseConfig(fs *flag.FlagSet) {
	fs.BoolVar(&disableAscend, "disable-ascend", false, "disable ascend device")
...
func (dev *Devices) GetNodeDevices(n corev1.Node) ([]*api.DeviceInfo, error) {
	...
	if !ok {
		return []*api.DeviceInfo{}, fmt.Errorf("annos not found %s", RegisterAnnos)
	}
	...
	if ok && len(devList) > 0 {
		...
		for _, dp := range devList {
			...
			for _, val := range dp {
				...
				value = fmt.Sprintf("%s%d-%s,", value, val.Idx, temp)
			}
			if len(value) > 0 {
...
func (dev *Devices) GenerateResourceRequests(ctr *corev1.Container) util.ContainerDeviceRequest {
	klog.Info("Counting AscendDevices devices")
